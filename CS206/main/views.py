...
def api_for_weather(place):
    ...
    result=requests.get('http://api.openweathermap.org/data/2.5/weather?q='+place+'&appid=79a8d704403b1c02ab4539f55e46470b')
    data=result.json()
    return data

def api_for_weather_2(lat,longi):
    
    result=requests.get('http://api.openweathermap.org/data/2.5/weather?lat='+str(lat)+'&lon='+str(longi)+'&appid=79a8d704403b1c02ab4539f55e46470b')
    data=result.json()
    return data
...
def get_soil_info(place,coord):
    ...
    #    result=requests.get('https://api.weatherbit.io/v2.0/history/agweather?lat='+str(coord[0])+'&lon='+str(coord[1])+'&start_date='+start_date+'&end_date='+end_date+'&key=98c690c9c15044978dfafa55736d6376')
        result=requests.get('https://api.weatherbit.io/v2.0/forecast/agweather?lat='+str(coord[0])+'&lon='+str(coord[1])+'&key=98c690c9c15044978dfafa55736d6376')
        data=result.json()
        return data
...
def geocoding(place):
    coord=[]
    result=requests.get('https://api.opencagedata.com/geocode/v1/json?q='+place+'&key=ef43c7676ea44473bbb598da7652b322 ')
    data=result.json()
...
def work(request):
    ...
    # handling the imports exports and production factors
            ...
            return render(request,'main/parallax.html',{'first':first_value,'second':sec_values,'third':third_values, 'temp_det':temp_det,'final':final_dict,'soil':soil_info,'finalout_main':final_main,'finalout_dec':final_dec,'ff':fin_dict})
        else:
            return render(request,'main/new.html',{'crop':crop})
